...
APP_BUILD = "2026.09.19 · Dynamic Strategist v7"
...
if mode=="Classic":
    try:
        ...
        with tabs[0]:
            ...
            st.caption(f"Build: {APP_BUILD} · Ask about the slate, the rules DFS LAB chose, or the portfolio it actually built. The conversation stays with this slate and contest.")
            ...
            # Conversation first; composer stays under the newest DFS LAB answer.
            if st.session_state["classic_ai_chat"]:
                for uq,ar in st.session_state["classic_ai_chat"][-8:]:
            ...
            with st.form("classic_intel_ai_form",clear_on_submit=True):
                cq=st.text_input("Reply to DFS LAB",placeholder="Ask a follow-up, challenge a take, or give DFS LAB a build instruction.")
                cask=st.form_submit_button("SEND  ↗",type="primary",use_container_width=True)

            _asked=(cq.strip() if cask and cq.strip() else quick_ask)
            if _asked:
                _ql=_asked.lower()

                # Dynamic AI router: interpret the user's meaning first, then apply only
                # clear instructions. Questions and challenges remain questions.
                _plan=classic_ai_action_plan(_asked,packet,st.session_state["classic_ai_chat"])
                _applied=[]
                if _plan is not None and bool(_plan.get("needs_clarification",False)):
                    ca=str(_plan.get("clarification") or "I need one detail before changing the build.")
                else:
                    if _plan is not None:
                        _applied=classic_apply_ai_actions(_plan.get("actions",[]),df)

                    # Conservative fallback when the full AI router is unavailable.
                    if _plan is None:
                        _player_row=None
                        _qnorm=re.sub(r"[^a-z0-9 ]+"," ",_ql)
                        for _,_pr in df.iterrows():
                            _nm=str(_pr.get("Name","")).strip()
                            if _nm and re.sub(r"[^a-z0-9 ]+"," ",_nm.lower()) in _qnorm:
                                _player_row=_pr; break
                        _pct_match=re.search(r"(\d{1,3}(?:\.\d+)?)\s*%",_ql)
                        _directive=any(x in _ql for x in ["set ","cut ","cap ","limit ","no more than","at most","maximum","max "])
                        if _player_row is not None and _pct_match is not None and _directive:
                            _pct=max(0.0,min(100.0,float(_pct_match.group(1))))
                            _pid=str(_player_row["ID"]); _pname=str(_player_row["Name"])
                            _cur=dict(st.session_state["strategy_master"].get(_pid,{}) or {})
                            _cur["Max Exposure"]=_pct
                            if float(_cur.get("Min Exposure",0) or 0)>_pct: _cur["Min Exposure"]=_pct
                            st.session_state["strategy_master"][_pid]=_cur
                            _applied=[f"{_pname} max exposure {_pct:.0f}%"]
                        _qb_action=re.search(r"(?:qb|quarterback).*?(?:cap|pool|field).*?(?:to|at|of)?\s*(\d{1,2})|(?:no\s+more\s+than|at\s+most)\s*(\d{1,2}).*?(?:qb|quarterback)",_ql)
                        if _qb_action and _directive:
                            _cap=int(next(x for x in _qb_action.groups() if x is not None))
                            _cap=max(1,min(32,_cap)); st.session_state["classic_qb_cap"]=_cap
                            _applied=[f"QB pool cap {_cap}"]

                    if any(x.startswith("QB pool cap") or x=="clear QB pool cap" for x in _applied):
                        classic_qb_ids,qb_plan_table,qb_plan=classic_qb_concentration_plan(df,thesis_table,entry_format)
                        classic_qb_ids,qb_plan_table,qb_plan=classic_apply_qb_cap(
                            classic_qb_ids,qb_plan_table,qb_plan,st.session_state.get("classic_qb_cap",0)
                        )
                        packet["qb_concentration"]=qb_plan
                        packet["qb_candidates"]=qb_plan_table.head(20).to_dict(orient="records") if qb_plan_table is not None and not qb_plan_table.empty else []

                    packet["active_rules"]={
                        "qb_pass_catchers":int(st.session_state["classic_qb_stack"]),
                        "bringback":st.session_state["classic_bringback"],
                        "min_salary":int(st.session_state["classic_min_salary"]),
                        "max_team":int(st.session_state["classic_max_team"]),
                        "max_game":int(st.session_state["classic_max_game"]),
                        "max_te":int(st.session_state["classic_max_te"]),
                        "no_dst_from_qb_game":bool(st.session_state["classic_no_dst"]),
                        "no_offense_vs_dst":bool(st.session_state["classic_no_off"]),
                        "allow_qb_rb":bool(st.session_state["classic_allow_qb_rb"])
                    }
                    with st.spinner("Reading contest → slate → portfolio → strategy → conversation…"):
                        ca=classic_ai_slate_answer(_asked,packet,st.session_state["classic_ai_chat"])
                    if _applied:
                        ca=ca.rstrip()+"\n\n**Applied to the next build:** "+", ".join(_applied)+"."
