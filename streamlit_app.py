...
APP_BUILD = "2026.09.24 · Results Command Center"
        ...
        with tabs[5]:
            ...
            if result is None or result.empty: st.info("Set your build, player takes and script, then generate lineups.")
                ...
                st.markdown(
                    ...
                    f"<div class='results-hub-title'>{len(result)} lineups are ready.</div>"
                    f"<div class='results-hub-sub'>The lineup portfolio, {_world_total} Game Worlds, and the Lineup Lab + DFS LAB Agent are all one tap away — no hunting down the page.</div></div>",
                    unsafe_allow_html=True
                )
                ...
                with result_views[0]:
                    ...
                    for _,_top_lr in result.head(3).iterrows():
                        _flex_names=" · ".join(str(_top_lr.get("FLEX"+str(i),"")) for i in range(1,6))
                        _card_html=("<div class='lineup-card'><div class='lineup-rank'>#%s &nbsp; <span>%s</span></div>" % (int(_top_lr["Rank"]),_top_lr["Rating"]) +
                                   "<div class='lineup-cpt'>CPT · %s</div>" % _top_lr["Captain"] +
                                   "<div class='lineup-flex'>%s</div>" % _flex_names +
                                   "<div class='lineup-meta'>Proj %.1f &nbsp; • &nbsp; $%s &nbsp; • &nbsp; %s &nbsp; • &nbsp; %s duplication risk</div></div>" % (_top_lr["Projection"],f"{int(_top_lr['Salary']):,}",_top_lr["Construction"],_top_lr["Dup Risk"]))
