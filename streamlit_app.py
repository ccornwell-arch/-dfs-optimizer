...
APP_BUILD = "2026.09.19 · Portfolio AI v5"
    ...
    try:
        ...
        with tabs[0]:
            ...
            if _asked:
                ...
                _qb_action=re.search(r"(?:no\s+more\s+than|max(?:imum)?(?:\s+of)?|limit(?:\s+(?:the\s+)?(?:qb|quarterback)\s+(?:field|pool))?\s+to|tighten(?:\s+(?:the\s+)?(?:qb|quarterback)\s+(?:field|pool))?\s+to)\s*(\d{1,2})",_ql)
