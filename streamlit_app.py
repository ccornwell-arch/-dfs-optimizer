...
APP_BUILD = "2026.09.22 · Lineup Coherence Engine"
...
def showdown_lineup_coherence(df, chosen, game_world=None):
    ...
    if fam=="pass_ceiling":
        ...
        if relevant.empty:
            hard.append(f"{game_world} has no {team} passing piece in the lineup.")
    elif fam=="rb_control":
        team=str(cfg.get("team",""))
        if p[(p["Team"].astype(str).eq(team)) & p["is_RB"]].empty:
            hard.append(f"{game_world} has no {team} running back to express the control script.")
    elif fam=="comeback":
        trail=str(cfg.get("trail",""))
        if p[(p["Team"].astype(str).eq(trail)) & (p["is_QB"]|p["is_passcatcher"])].empty:
            hard.append(f"{game_world} lacks a {trail} passing piece for the comeback.")
    elif fam=="qb_ceiling":
        player=str(cfg.get("player",""))
        if player and not p["Name"].astype(str).eq(player).any():
            hard.append(f"{game_world} does not contain {player}.")
...
Be conversational, concise, and useful. Sound like a strong DFS partner sitting next to the user. Avoid canned filler such as 'I can inspect...' when evidence already answers the question. If evidence is insufficient, say exactly what is missing and give the best supported observation."""
                ...
                cols=["Rank","Rating","Rating Score","Projection","Coherence Score","Lineup Story","Coherence Flags","Salary","Salary Left","Captain","Captain Pos","CPT Own","Construction","Dup Risk","Projection Grade","Captain Grade","Correlation Grade","Leverage Grade","Duplication Grade","Story","CPT","FLEX1","FLEX2","FLEX3","FLEX4","FLEX5"]
