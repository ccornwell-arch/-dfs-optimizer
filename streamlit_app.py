...
APP_BUILD = "2026.09.24 · Live Availability + Scenario Alignment"
...
def contest_world_weights(entry_format, field_size, script="Neutral", script_team="", influence=50):
    # Portfolio exploration weights, generated from the CURRENT uploaded matchup.
    # A directional user thesis must shape the worlds themselves, not merely the
    # player objective after a contradictory world has already been sampled.
    if not GAME_WORLDS:
    ...
    # Directional alignment. At 100 influence, contradictory lead worlds become
    # nearly impossible; at lower influence, DFS LAB still explores alternatives.
    if script_team and script in ["Team dominates","Team plays from ahead","Team wins close","Team passing comeback"]:
        s=float(np.clip(influence,0,100))/100.0
        for name,cfg in GAME_WORLDS.items():
            fam=cfg.get("family")
            target=1.0
            if script in ["Team dominates","Team plays from ahead","Team wins close"]:
                if fam=="comeback":
                    target=5.0 if cfg.get("lead")==script_team else 0.05
                elif fam=="rb_control":
                    target=4.0 if cfg.get("team")==script_team else 0.10
                elif fam in ["pass_ceiling","qb_ceiling"]:
                    target=2.4 if cfg.get("team")==script_team else 0.25
                elif fam=="shootout": target=0.12 if script=="Team dominates" else 0.35
                elif fam=="low": target=0.45
            elif script=="Team passing comeback":
                if fam=="comeback":
                    target=5.0 if cfg.get("trail")==script_team else 0.08
                elif fam in ["pass_ceiling","qb_ceiling"]:
                    target=3.5 if cfg.get("team")==script_team else 0.45
                elif fam=="shootout": target=0.55
                elif fam=="rb_control":
                    target=0.20 if cfg.get("team")==script_team else 1.6
            w[name]*=((1.0-s)+s*target)
...
def choose_game_world(rng, entry_format, field_size, script, script_team="", influence=50):
    weights=contest_world_weights(entry_format, field_size, script, script_team, influence)
...
def generate_showdown_lineups(df, field_size, payout_style, count, attempts, min_salary, max_salary,
    ...
    for attempt in range(attempts):
        ...
        game_world=choose_game_world(rng,entry_format,field_size,script,script_team,world_influence)
...
if mode=="Classic":
    ...
    try:
        ...
        with tabs[4]:
            ...
            st.caption("The scenario engine changes projections, construction AND Game World sampling before grading. Directional scripts such as GB dominates now strongly favor worlds where that selected team leads; alternative worlds shrink as Scenario influence rises.")
