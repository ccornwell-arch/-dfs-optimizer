...
APP_BUILD = "2026.09.19 · Portfolio AI v6"
    ...
    try:
        ...
        with tabs[0]:
            ...
            if _asked:
                ...
                # Identify a player named in the instruction/question.
                _player_row=None
                ...
                for _,_pr in df.iterrows():
                    ...
                    if _nm and re.sub(r"[^a-z0-9 ]+"," ",_nm.lower()) in _qnorm:
                        _player_row=_pr
                        break
                if _player_row is None:
                    _qwords=set(_qnorm.split())
                    _hits=[]
                    for _,_pr in df.iterrows():
                        _nm=str(_pr.get("Name","")).strip()
                        _parts=[x for x in re.sub(r"[^a-z0-9 ]+"," ",_nm.lower()).split() if len(x)>=3]
                        if _parts and _parts[-1] in _qwords:
                            _hits.append(_pr)
                    if len(_hits)==1:
                        _player_row=_hits[0]
                ...
                _is_exp_limit=(
                    _player_row is not None and _pct_match is not None
                    and any(x in _ql for x in ["exposure","owned","ownership"])
                    and any(x in _ql for x in ["no more than","maximum"," max ","limit","cap","at most"])
                )
                _qb_action=re.search(r"(?:no\s+more\s+than|want\s+no\s+more\s+than|don['’]?t\s+want\s+(?:more\s+than|over)|do\s+not\s+want\s+(?:more\s+than|over)|max(?:imum)?(?:\s+of)?|limit(?:\s+(?:the\s+)?(?:qb|quarterback)\s+(?:field|pool))?\s+to|tighten(?:\s+(?:the\s+)?(?:qb|quarterback)\s+(?:field|pool))?\s+to)\s*(\d{1,2})",_ql)
                _clear_qb=(("qb" in _ql or "quarterback" in _ql) and any(x in _ql for x in ["clear cap","remove cap","no cap","reset qb"]))

                if _is_exp_limit:
                    _pct=max(0.0,min(100.0,float(_pct_match.group(1))))
                    _pid=str(_player_row["ID"])
                    _pname=str(_player_row["Name"])
                    _cur=dict(st.session_state["strategy_master"].get(_pid,{}) or {})
                    _cur["Max Exposure"]=_pct
                    if float(_cur.get("Min Exposure",0) or 0)>_pct:
                        _cur["Min Exposure"]=_pct
                    st.session_state["strategy_master"][_pid]=_cur
                    ca=(f"Set **{_pname} max exposure to {_pct:.0f}%** for your portfolio. "
                        f"That changes **your lineup exposure**, not his projected field ownership. "
                        "Rebuild the lineups and DFS LAB will use the new cap.")
                elif _qb_action and ("qb" in _ql or "quarterback" in _ql):
                    _cap=max(1,min(32,int(_qb_action.group(1))))
                    st.session_state["classic_qb_cap"]=_cap
                    classic_qb_ids,qb_plan_table,qb_plan=classic_apply_qb_cap(classic_qb_ids,qb_plan_table,qb_plan,_cap)
                    ...
                    _names=", ".join(qb_plan.get("names",[])[:10])
                    ca=(f"Yes. I set the QB pool to **no more than {_cap}** for this slate. "
                        f"DFS LAB will now build only from the top evidence-backed QB paths inside that cap"
                        +(f": **{_names}**." if _names else ".")+
                        " This is an actual build rule now, not just advice. Rebuild the lineups and the cap will be applied.")
                elif _clear_qb:
                    st.session_state["classic_qb_cap"]=0
                    classic_qb_ids,qb_plan_table,qb_plan=classic_qb_concentration_plan(df,thesis_table,entry_format)
                    packet["qb_concentration"]=qb_plan
                    packet["qb_candidates"]=qb_plan_table.head(20).to_dict(orient="records") if qb_plan_table is not None and not qb_plan_table.empty else []
                    ca="QB cap cleared. DFS LAB is back to the slate-driven evidence cutoff rather than a user-set maximum."
                else:
