...
APP_BUILD = "2026.09.21 · Dynamic Worlds + Primary QB Guard"
...
def prepare_showdown_pool(dk_file, ss_file=None):
    ...
    # Showdown QB guard: DraftKings can list multiple QBs from the same team, but a
    # normal build should not treat the backup as a viable player just because his
    # historical model projection is non-zero. The slate salary is the strongest
    # current-market starter signal available in the DK file, with season scoring and
    # model projection used as tie-breakers.
    df["Primary QB"]=False
    df["Auto Excluded Reason"]=""
    for team in teams:
        qidx=df.index[df["Team"].eq(team)&df["is_QB"]].tolist()
        if not qidx:
            continue
        primary=max(
            qidx,
            key=lambda i: (
                float(df.loc[i,"FlexSalary"]),
                float(df.loc[i,"AvgPointsPerGame"]),
                float(df.loc[i,"My Proj"])
            )
        )
        df.loc[primary,"Primary QB"]=True
        for i in qidx:
            if i!=primary:
                df.loc[i,"ActiveForBuild"]=False
                df.loc[i,"Auto Excluded Reason"]="Backup QB — DFS LAB keeps only the primary QB active by default"
